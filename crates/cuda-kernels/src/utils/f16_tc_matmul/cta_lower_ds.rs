...
#[expect(clippy::too_many_arguments, reason = "CUDA ABI uses explicit buffers")]
pub(super) fn cta_matmul_windowed_lower_ds_sparse_body(
    a: &[u16],
    b_t: &[u16],
    probs: &[u16],
    softmax_d: &[f32],
    ...
    let probability_scale = super::sparse_tile::attention_tile_scale(
        tile_scales,
        tile.batch,
        dims.m,
        tile.row_base,
...
fn store_pair(
    ...
    if row0 < dims.m && row1 == row0 && col0 + 1 == col1 && col1 < dims.n && col1 <= row0 {
        ...
        let grad0 = probability_scale * cvt_f32_f16(packed_probs as u16) * (dot0 - d);
        let grad1 = probability_scale * cvt_f32_f16((packed_probs >> 16) as u16) * (dot1 - d);
...
fn store_one(
    ...
    if row < dims.m && col < dims.n && col <= row {
        ...
        let grad = probability_scale
            * cvt_f32_f16(load_f16_global_bits_read_only(probs.as_ptr(), offset))
            * (dot - d);
        unsafe {
